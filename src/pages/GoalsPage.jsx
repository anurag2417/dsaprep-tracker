...
export const GoalsPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in">
      ...
      <form
        onSubmit={handleAdd}
        className="glass-panel p-6 rounded-2xl flex flex-wrap gap-5 items-end"
      >
      ...
      <div className="space-y-4">
        {goals.length === 0 ? (
          ...
          goals.map((goal) => (
            <div
              key={goal.id}
              className={`flex items-center justify-between p-5 rounded-2xl transition-all duration-300 ${goal.completed ? "bg-emerald-500/10 border border-emerald-500/20 opacity-60" : "glass-panel hover:-translate-y-1"}`}
            >
              <div className="flex items-center space-x-5">
                <input
                  ...
                  className="h-6 w-6 text-blue-500 rounded border-white/20 bg-white/5 focus:ring-blue-500 focus:ring-offset-[#0a0a0a] cursor-pointer transition-colors"
                />
                <div>
                  <h3
                    className={`text-lg font-bold ${goal.completed ? "line-through text-gray-500" : "text-gray-100"}`}
                  >
                    {goal.text}
                  </h3>
                  <div className="flex gap-3 mt-1 text-sm font-medium">
                    <span className="text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md">
                      Topic: {goal.topic}
                    </span>
